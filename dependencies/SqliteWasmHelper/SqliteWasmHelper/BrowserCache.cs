...
namespace SqliteWasmHelper
{
    ...
    public sealed class BrowserCache : IAsyncDisposable, IBrowserCache
    {
        ...
        /// </summary>
        /// <param name="filename">The database filename.</param>
        /// <returns>A string with url or null if not exists.</returns>
        public async Task<string?> GenerateDownloadUrlAsync(string filename)
        {
            var module = await moduleTask.Value;
            return await module.InvokeAsync<string?>("generateDownloadUrl", filename);
        }
        ...
        /// </summary>
        /// <param name="filename">The database filename.</param>
        /// <returns>A value indicating whether the operation wvas successful.</returns>
        public async Task<bool> DeleteAsync(string filename)
        {
            var module = await moduleTask.Value;
            return await module.InvokeAsync<bool>("removeDatabase", filename);
        }
    }
}
