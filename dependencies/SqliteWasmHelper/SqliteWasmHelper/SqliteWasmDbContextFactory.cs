...
namespace SqliteWasmHelper
{
    ...
    public class SqliteWasmDbContextFactory<TContext> : IDbContextExtendedFactory<TContext>
        where TContext : DbContext
    {
        ...
        public async Task<TContext> CreateDbContextAsync()
        {
            ...
            if (!init)
            {
                ...
                // TODO: Using of ef core migrate or not should be define in options
        ...
        private async Task CheckForStartupTaskAsync()
        {
            if (startupTask != null)
            {
                ...
                // Sometimes, if two tasks are inside at the "same time' startupTask may be null here
                startupTask?.Dispose();
                startupTask = null;
            }
        }
        ...
        private async Task FlushDatabase(TContext ctx)
        {
            await ctx.Database.CloseConnectionAsync();
            await CheckForStartupTaskAsync();
