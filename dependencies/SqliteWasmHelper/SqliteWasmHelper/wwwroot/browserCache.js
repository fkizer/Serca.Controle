...
export async function generateDownloadLink(parent, file) {
    ...
    const backupPath = `${file}`;
    const cachePath = `/data/cache/${file.substring(0, file.indexOf('_bak') > 0 ? file.indexOf('_bak') : file.length)}`;
    const db = window.sqlitedb;
    const resp = await db.cache.match(cachePath);
...
export async function removeDatabase(file) {
    const cachePath = `/data/cache/${file.substring(0, file.indexOf('_bak') > 0 ? file.indexOf('_bak') : file.length)}`;
    const db = window.sqlitedb;
    return await db.cache.delete(cachePath);
}